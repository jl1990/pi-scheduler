...
async function harness(root, exec) {
	const stateDir = await fs.mkdtemp("/tmp/pi-scheduler-runtime-");
	...
	const source = stripTypeScriptTypes(readFileSync(path.join(root, "extensions/scheduler/index.ts"), "utf8"))
		.replace(/^import .*;\s*$/gm, "").replace("export default function schedulerExtension", "function schedulerExtension");
	const events = {}, tools = {}, commands = {}, wakes = [], messages = [];
	const context = { cwd: root, hasUI: false, isIdle: () => true, sessionManager: { getSessionFile: () => path.join(stateDir, "session.jsonl") } };
	const pi = {
		on: (name, fn) => events[name] = fn, registerTool: tool => tools[tool.name] = tool,
		registerCommand: (name, command) => commands[name] = command, registerMessageRenderer: () => {},
		sendUserMessage: (text, options) => wakes.push({ text, options }), sendMessage: (message, options) => messages.push({ message, options }), exec,
	};
	const sandbox = { require: localRequire, process: { ...process, env: { ...process.env, PI_SCHEDULER_STATE_FILE: stateFile } }, console,
		setTimeout, clearTimeout, setInterval, clearInterval, Cron: localRequire("croner").Cron, randomUUID,
		homedir: () => stateDir, join: path.join, StringEnum: values => ({ enum: values }), Text: class {},
	...
	return { events, tools, commands, wakes, messages, stateFile, context, start: () => events.session_start({}, context),
